    ...
    <div className="fixed inset-0 z-50">
      ...
      <aside className="absolute inset-y-0 end-0 flex w-[26rem] max-w-[92%] flex-col border-s border-border bg-surface shadow-frame">
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-5">
          {cart.lines.length === 0 && (
            <p className="text-sm text-muted-foreground">העגלה ריקה כרגע.</p>
          )}
          {cart.lines.map((line) => {
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {shekel(p.price)} / {p.unit}
                  </p>
                  <div className="mt-2 flex items-center gap-2">
                    <button
                      onClick={() => cart.setQty(p.id, line.qty - 1)}
                      className="rounded-md border border-border p-1"
                      aria-label="הפחתה"
                    >
                      <Minus className="size-3.5" />
                    </button>
                    <span className="w-7 text-center text-sm font-bold text-platinum">
                      {line.qty}
                    </span>
                    <button
                      onClick={() => cart.setQty(p.id, line.qty + 1)}
