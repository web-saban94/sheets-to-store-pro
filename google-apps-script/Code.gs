...
var DEFAULT_RULES = [
  ...
  ["coupon_SABAN10", 0.1, "קופון 10% הנחה"],
...
function seedCatalog_() {
  ...
  if (brands.getLastRow() < 2) {
    ...
    brands.getRange(2, 1, list.length, 4).setValues(
  ...
  if (products.getLastRow() < 2) {
    products.getRange(2, 1, 2, SHEETS.Products.length).setValues([
      [
        ...
        "שק",
        "",
        "מלט אפור איכותי לעבודות בטון וטיח",
        "התקשרות 3-4 שעות, אשפרה 28 יום",
        'כ-1 מ"ר בעובי 5 ס"מ',
        "ערבוב במערבל, יישום ביעה או משאבה",
        "מבצע שבועי",
        true,
      ],
      [
        ...
        "פח",
        "",
        "צבע אקרילי מט לפנים ולחוץ",
        "יבש למגע שעה, שכבה נוספת 4 שעות",
        'עד 10 מ"ר לליטר',
        "רולר/מברשת/ריסוס, 2 שכבות",
        "מבצע שבועי",
        true,
      ],
    ]);
...
function doGet(e) {
  ...
  try {
    ...
    if (action === "customer") {
      ...
      var found = rows_("CRM_Customers").filter(function (c) {
        return String(c.phone) === phone;
      });
...
function createCustomer_(p) {
  var existing = rows_("CRM_Customers").filter(function (c) {
    return String(c.phone) === String(p.phone);
  });
  if (existing.length)
    return { ok: true, message: "הלקוח כבר קיים במערכת.", customer: existing[0] };
  append_("CRM_Customers", {
    customerId: "C-" + new Date().getTime(),
    createdAt: new Date().toISOString(),
    name: p.name || "",
    ...
    if (String(data[i][1]) === String(p.sku)) {
      sh.getRange(i + 1, 1, 1, headers.length).setValues([
        headers.map(function (h, idx) {
          return p[h] !== undefined ? p[h] : data[i][idx];
        }),
      ]);
